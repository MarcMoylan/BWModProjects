		...
		for(j=0;j<Clouds[i].Touching.length;j++)
		{
			...
			for(k=0;k<Served.length;k++)
				if (Served[k] == Clouds[i].Touching[j])
					break;
			if (k >= Served.length)	{
                    Pawn(Clouds[i].Touching[j]).GiveHealth(Damage, Pawn(Clouds[i].Touching[j]).HealthMax);
...
simulated function CloudDie(G28Cloud C)
{
	local int i;
...
simulated function SpawnCloud(vector V)
{
	...
	if (Owner != None)
		C = Spawn(class'G28Cloud',self,,V, Rotator(Owner.Velocity));
	else if (Clouds.length>0 && Clouds[Clouds.length-1] != None)
		C = Spawn(class'G28Cloud',self,,V, Rotator(V - Clouds[Clouds.length-1].Location));
	else if (Instigator != None)
		C = Spawn(class'G28Cloud',self,,V, Rotator(V - Instigator.Location));
	else
		C = Spawn(class'G28Cloud',self,,V);
	if (C != None)
	{
...
{
     Damage=1
     DamageType=Class'BWBP_SKC_Fix.DTG28Gas'
     bHidden=True
