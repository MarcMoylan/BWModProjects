...
//=============================================================================
class MGLSecondaryFire extends BallisticProjectileFire;
...
simulated function bool AllowFire()
{
	...
	else if (BW.MagAmmo < AmmoPerFire)
	{
		...
		if (MGLLauncher(BW).bRemoteGrenadeOut)
			return true;
	...
	if (MGLLauncher(BW).bRemoteGrenadeOut)
	{
		if (Weapon.Role == ROLE_Authority)
		{
			...
			if(LastGrenade == None || LastGrenade.IsInState('NetTrapped'))
				MGLLauncher(BW).UpdateGrenadeStatus(false); //Alert gun we've detonated grenade
		}
...
function SpawnProjectile (Vector Start, Rotator Dir)
{
	...
	MGLLauncher(BW).UpdateGrenadeStatus(true); //Alert gun we've fired grenade
}
...
defaultproperties
{
     ...
     ProjectileClass=Class'BWBP_SKC_Fix.MGLGrenadeRemote'
