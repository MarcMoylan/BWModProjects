...
simulated function Notify_CockStart()
{
	...
	if (bReady)
	{
		...
		Grenades -= 1;
		if (Grenades <= 0)
			PUGSecondaryFire(FireMode[1]).bLoaded = false;
		WeaponModes[0].bUnavailable=false;
		WeaponModes[2].bUnavailable=false;
		CurrentWeaponMode=LastWeaponMode;
		WeaponModes[3].bUnavailable=true;
...
function ServerStartReload (optional byte i)
{
	...
	if (ReloadState != RS_None)
		return;
	if (MagAmmo >= default.MagAmmo && (Ammo[1].AmmoAmount < 1 || Grenades >= 3)) //Pri Full - Alt Can't
		return;

	if (Grenades >= 3 && Ammo[0].AmmoAmount < 1) //Can't reload pri
		return;
...
//     SightOffset=(X=-20.000000,Y=4.500000,Z=7.000000)
     ...
     AimSpread=(X=(Min=-386.000000,Max=386.000000),Y=(Min=-386.000000,Max=386.000000))
     ChaosAimSpread=(X=(Min=-3072.000000,Max=3072.000000),Y=(Min=-3072.000000,Max=3072.000000))
     ...
     FireModeClass(0)=Class'BWBP_SKC_Fix.PUGPrimaryFire'
     FireModeClass(1)=Class'BWBP_SKC_Fix.PUGSecondaryFire'
